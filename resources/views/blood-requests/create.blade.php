                ...
                <form method="POST" action="{{ route('blood-requests.store') }}">
                    ...
                    <div class="grid gap-6">
                        <div>
                            ...
                            <select id="blood_type_needed" name="blood_type_needed" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm">
                                ...
                                @foreach($bloodTypes as $type)
                                    <option value="{{ $type }}" @selected(old('blood_type_needed') === $type)>{{ $type }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input id="quantity_requested" name="quantity_requested" type="number" min="1" value="{{ old('quantity_requested') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm" />
                        ...
                        <div>
                            ...
                            <select id="urgency_level" name="urgency_level" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm">
                                ...
                                @foreach($urgencyLevels as $level)
                                    <option value="{{ $level }}" @selected(old('urgency_level') === $level)>{{ ucfirst($level) }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="notes" name="notes" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 sm:text-sm">{{ old('notes') }}</textarea>
                            <x-input-error :messages="$errors->get('notes')" class="mt-2" />
                        </div>
                    </div>

                    <div class="mt-6 flex justify-end gap-3">
                        <a href="{{ route('blood-requests.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Cancel</a>
                        <button type="submit" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md text-sm font-medium text-white hover:bg-red-700">Submit Request</button>
                    </div>
                </form>
            </div>
        </div>
